...
export default function Book() {
  ...
  async function getBookData() {
    let {data} = await axios.get(`http://localhost:5001/book/books/${allURLParams.id}`, {
      headers: {
  ...
  const issueBook = async (e) => {
    ...
    try {
      const {data} = await axios.post('http://localhost:5001/book/issue' ,{bookId:bookData._id,issuedDurationInDays:duration}, {
          headers: {
            token: localStorage.getItem('userToken')
          },
        });
          ...
          <div className="col-10 px-lg-5 px-2 my-3">
            <div className="row">
                <div className="col-lg-4">
                  <div className='p-5'>
                  <img className='w-100 rounded-2' src={`http://localhost:5001/${bookData.bookPhoto}`} alt="" />
